  ...
  selector: 'page-azkarofday',
    ...
    animations: [
      ...
      trigger('decrease', [
        ...
        transition('inactive => active', animate(`${animationTimeOut}ms ease-in`)),
        transition('active => inactive', animate(`${animationTimeOut}ms ease-out`))
      ])
    ]
})
export class AzkarofdayPage implements AfterViewInit{
  ...
  private getPageAzkar() {
    ...
    this.azkars = this.azkarsProvider[ZekrTypeData[this.azkarType]].map(zekr => {
  ...
  private formatZekr(text) {
    return text.replace(/اللَّهُ|اللّهُ|ٱللَّهُ|لله|اللّهـمَّ/g,'<span class="holy">$&</span>')
  }
